...
syn region ciscoClassMaps start=+^class-map\s+ skip=+^\(!\n\)\?class-map\s+ end=+^\S+me=s-1 fold contains=ciscoClassMap
syn region ciscoClassMap contained start=+^class-map\s+ end=+^\S+me=s-1 fold contains=ciscoIpAddr,ciscoIfName,ciscoComment
syn region ciscoPolicyMaps start=+^policy-map\s+ skip=+^\(!\n\)\?policy-map\s+ end=+^\S+me=s-1 fold contains=ciscoPolicyMap
syn region ciscoPolicyMap contained start=+^policy-map\s+ end=+^\S+me=s-1 fold contains=ciscoIpAddr,ciscoIfName,ciscoComment
